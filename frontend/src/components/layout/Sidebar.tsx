...
export default function Sidebar() {
  return (
    <aside className="fixed top-0 left-0 w-58 h-screen bg-[--color-surface] border-r border-[--color-border] flex flex-col z-50"
      style={{ width: '232px' }}>

      {/* Logo */}
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-[--color-border]">
        <div className="w-8 h-8 rounded-lg bg-[--color-accent] text-white flex items-center justify-center text-sm font-bold tracking-tight flex-shrink-0">
          O
        </div>
        <span className="text-sm font-semibold tracking-widest text-[--color-text-primary]">OPTIMA</span>
      </div>
      ...
      <div className="flex items-center gap-2 px-5 py-3.5 border-t border-[--color-border]">
        <div className="w-1.5 h-1.5 rounded-full bg-[--color-border-2] flex-shrink-0" />
        <span className="text-xs text-[--color-text-tertiary]">Backend offline</span>
      </div>
    </aside>
  );
}
