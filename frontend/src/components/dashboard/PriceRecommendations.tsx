  ...
  return (
    <div className="bg-[--color-surface] border border-[--color-border] rounded-2xl shadow-xs overflow-hidden">
      <div className="flex items-center gap-2.5 px-6 py-5 border-b border-[--color-border]">
        <h2 className="text-sm font-semibold text-[--color-text-primary]">AI Price Recommendations</h2>
      ...
      {/* Header row */}
      <div className="grid px-6 py-2 border-b border-[--color-border] bg-[--color-surface-2] text-[11px] font-semibold uppercase tracking-widest text-[--color-text-tertiary]"
        style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr 1.2fr 80px' }}>
        <span>Product</span><span>Current</span><span>Recommended</span><span>Change</span><span>Confidence</span><span></span>
      </div>
      ...
      {recs.map((r, i) => {
        ...
        return (
          <div
            ...
            style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr 1.2fr 80px', animationDelay: `${i * 40}ms` }}
          >
            <span className="text-[13.5px] text-[--color-text-primary] truncate pr-4">{r.name}</span>
            <span className="text-sm text-[--color-text-secondary] font-mono">${r.current_price.toFixed(2)}</span>
            <span className="text-sm text-[--color-text-secondary] font-mono">${r.recommended.toFixed(2)}</span>
            <span className={`text-xs font-semibold font-mono ${diff >= 0 ? 'text-[--color-success]' : 'text-[--color-danger]'}`}>
              {diff >= 0 ? '+' : ''}{pct.toFixed(1)}%
            </span>
            <div className="flex items-center gap-2 pr-2">
              <div className="flex-1 h-1 bg-[--color-surface-2] rounded-full overflow-hidden">
                <div className="h-full bg-[--color-accent] rounded-full transition-all duration-500" style={{ width: `${r.confidence * 100}%` }} />
              </div>
              <span className="text-[11px] text-[--color-text-tertiary] font-mono w-7 text-right">{(r.confidence * 100).toFixed(0)}%</span>
            </div>
