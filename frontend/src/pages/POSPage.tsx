...
export default function POSPage() {
  ...
  return (
    <div className="flex flex-col gap-5 max-w-[1400px]" style={{ height: 'calc(100vh - 60px - 56px)', minHeight: '600px' }}>
      ...
      <div className="grid gap-4 flex-1 min-h-0" style={{ gridTemplateColumns: '1fr 320px' }}>
        <div className="min-h-0 overflow-hidden flex flex-col bg-[--color-surface] border border-[--color-border] rounded-2xl shadow-xs p-4">
          <ProductGrid onAdd={(p: Product) => addItem(p)} />
        </div>
      ...
      {/* Transaction log */}
      <div className="bg-[--color-surface] border border-[--color-border] rounded-2xl shadow-xs overflow-hidden flex-shrink-0">
        <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-[--color-border]">
        ...
        {/* Log head */}
        <div className="grid px-5 py-2 bg-[--color-surface-2] border-b border-[--color-border] text-[10px] font-semibold uppercase tracking-widest text-[--color-text-tertiary]"
